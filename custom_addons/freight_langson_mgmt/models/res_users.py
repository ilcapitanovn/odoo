    ...
    def write(self, vals):
        ...
        try:
            if self.id != self.env.ref('base.user_admin').id:
                menu_freight_main = self.env.ref('freight_langson_mgmt.freight_langson_mgmt_main_menu', False)
                branch_langson = self.env.ref('seenpo_multi_branch_base.seenpo_branch_langson', False)
                if menu_freight_main and branch_langson and self.branch_ids:
                    if branch_langson.id not in self.branch_ids.ids:
            ...
            print("AccessError occurred when write restrict user access to Lang Son (Freight) menu: ", str(e))
